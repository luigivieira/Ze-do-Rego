...
public class FireExtinguisherTalkAction : TalkAction {
		...
		if(choices.Length > 0 ){
			if(choices.Length == 1) {
				...
				foreach (Dialogue.Choice choice in choices) {
					if((choice.userData.Contains("observed") && LevelManager.Instance().observedFireExtinguisher) || 
					   (!choice.userData.Contains("observed") && !LevelManager.Instance().observedFireExtinguisher)){
